...
export default function NoteEditor() {
  ...
  const handleSave = async (values) => {
    try {
      ...
      const textContent = content.replace(/<[^>]*>/g, ''); // Strip HTML for plain text
      ...
      if (response.ok) {
        ...
        message.success(isEdit ? 'Note updated successfully' : 'Note created successfully');
        router.push(`/dashboard/notes/${savedNote._id}`);
      } else {
